...
TYPED_TEST(SVSTieredIndexTest, manageIndexOwnership) {
    ...
    auto dummy_job = [](AsyncJob *job) {
        auto *my_index = reinterpret_cast<TieredSVSIndex<TEST_DATA_T, TEST_DIST_T> *>(job->index);
    ...
    auto job1 =
        new (allocator) AsyncJob(allocator, SVS_INSERT_VECTOR_JOB, dummy_job, tiered_index);
    auto job2 =
        new (allocator) AsyncJob(allocator, SVS_INSERT_VECTOR_JOB, dummy_job, tiered_index);
...
TYPED_TEST(SVSTieredIndexTestBasic, deleteFromSVSMulti) {
    ...
    ASSERT_EQ(((SVSRepairJob *)(mock_thread_pool.jobQ.front().job))->node_id, 1);
    ASSERT_EQ(((SVSRepairJob *)(mock_thread_pool.jobQ.front().job))->level, 0);
    ...
    ASSERT_EQ(tiered_index->deleteLabelFromSVS(1), 2);
    ASSERT_EQ(mock_thread_pool.jobQ.size(), 2);
...
TYPED_TEST(SVSTieredIndexTestBasic, deleteFromSVSMultiLevels) {
    // Create TieredSVS index instance with a mock queue.
    size_t dim = 4;
    ...
    do {
        ...
        if (tiered_index->getSVSIndex()->getGraphDataByInternalId(vec_id)->toplevel > 0) {
    ...
    ASSERT_EQ(tiered_index->deleteLabelFromSVS(vec_id), 1);
    ASSERT_EQ(tiered_index->getSVSIndex()->getGraphDataByInternalId(vec_id)->toplevel, 1);
    // This should be an array of length 1.
    auto &level_one = tiered_index->getSVSIndex()->getElementLevelData(vec_id, 1);
    ...
    ASSERT_EQ(((SVSRepairJob *)(mock_thread_pool.jobQ.front().job))->level, 1);
    ASSERT_EQ(((SVSRepairJob *)(mock_thread_pool.jobQ.front().job))->node_id, level_one.links[0]);
}

TYPED_TEST(SVSTieredIndexTest, deleteFromSVSWithRepairJobExec) {
    // Create TieredSVS index instance with a mock queue.
    size_t n = 1000;
    size_t dim = 4;
    bool isMulti = TypeParam::isMulti();
    ...
    for (size_t i = 0; i < n; i++) {
        GenerateAndAddVector(tiered_index->getSVSIndex(), dim, i, i);
    }

    // Delete vectors one by one and run the resulted repair jobs.
    while (tiered_index->getSVSIndex()->getNumMarkedDeleted() < n) {
        // Choose the current entry point each time (it should be modified after the deletion).
        idType ep = tiered_index->getSVSIndex()->safeGetEntryPointState().first;
        auto incoming_neighbors =
            tiered_index->getSVSIndex()->safeCollectAllNodeIncomingNeighbors(ep);
        ASSERT_EQ(tiered_index->deleteLabelFromSVS(ep), 1);
        ASSERT_EQ(mock_thread_pool.jobQ.size(), incoming_neighbors.size());
        ASSERT_EQ(tiered_index->getSVSIndex()->checkIntegrity().connections_to_repair,
                  mock_thread_pool.jobQ.size());
        ASSERT_NE(tiered_index->getSVSIndex()->safeGetEntryPointState().first, ep);

        // Execute synchronously all the repair jobs for the current deletion.
        while (!mock_thread_pool.jobQ.empty()) {
            idType repair_node_id = ((SVSRepairJob *)(mock_thread_pool.jobQ.front().job))->node_id;
            auto repair_node_level = ((SVSRepairJob *)(mock_thread_pool.jobQ.front().job))->level;

            tiered_index->getSVSIndex()->repairNodeConnections(repair_node_id, repair_node_level);
            ElementLevelData &node_level = tiered_index->getSVSIndex()->getElementLevelData(
        ...
        ASSERT_EQ(tiered_index->getSVSIndex()->checkIntegrity().connections_to_repair, 0);
    }
}

TYPED_TEST(SVSTieredIndexTest, manageIndexOwnershipWithPendingJobs) {
    // Create TieredSVS index instance with a mock queue.
    size_t dim = 4;
    bool isMulti = TypeParam::isMulti();
    ...
    // Add two vectors directly to SVS, and remove one vector to create a repair job.
    GenerateAndAddVector<TEST_DATA_T>(tiered_index->backendIndex, dim, 0, 0);
    GenerateAndAddVector<TEST_DATA_T>(tiered_index->backendIndex, dim, 1, 1);
    ASSERT_EQ(tiered_index->deleteLabelFromSVS(0), 1);
    ASSERT_EQ(tiered_index->idToRepairJobs.size(), 1);
...
TYPED_TEST(SVSTieredIndexTestBasic, AdHocSingle) {
    size_t dim = 4;
    ...
    VecSimIndex_AddVector(svs_index, vec1, 1); // vec1 is inserted to SVS only.
    VecSimIndex_AddVector(flat_index, vec2, 2); // vec2 is inserted to flat only.

    // vec3 is inserted to both SVS and flat, simulating a vector that was inserted
    // to SVS and not yet removed from flat.
    VecSimIndex_AddVector(svs_index, vec3, 3);
...
TYPED_TEST(SVSTieredIndexTestBasic, AdHocMulti) {
    size_t dim = 4;
    ...
    // vec1_* are inserted to SVS only.
    ...
    // vec3_* are inserted to both SVS and flat (some to SVS only, some to flat only)
    ...
    // vec4_* are inserted to both SVS and flat with some overlap.
    ...
    // Insert vectors to the tiered index.
    VecSimIndex_AddVector(svs_index, vec1_1, 1);
    VecSimIndex_AddVector(svs_index, vec1_2, 1);
    VecSimIndex_AddVector(svs_index, vec1_3, 1);
    ...
    VecSimIndex_AddVector(svs_index, vec3_1, 3);
    VecSimIndex_AddVector(flat_index, vec3_2, 3);
    VecSimIndex_AddVector(svs_index, vec3_3, 3);

    VecSimIndex_AddVector(svs_index, vec4_1, 4);
    VecSimIndex_AddVector(svs_index, vec4_2, 4);
...
TYPED_TEST(SVSTieredIndexTest, parallelInsertAdHoc) {
    ...
    auto parallel_adhoc_search = [](AsyncJob *job) {
        ...
        bool isMulti =
            reinterpret_cast<TieredSVSIndex<TEST_DATA_T, TEST_DIST_T> *>(search_job->index)
...
TYPED_TEST(SVSTieredIndexTestBasic, deleteVectorMulti) {
    ...
    ASSERT_EQ(reinterpret_cast<SVSInsertJob *>(mock_thread_pool.jobQ.front().job)->id,
              invalidJobsCounter++);
    mock_thread_pool.thread_iteration();
    ASSERT_EQ(mock_thread_pool.jobQ.size(), 0);
    ...
    ASSERT_EQ(mock_thread_pool.jobQ.size(), 2);
    ASSERT_EQ(mock_thread_pool.jobQ.front().job->jobType, SVS_REPAIR_NODE_CONNECTIONS_JOB);
    ASSERT_EQ(reinterpret_cast<SVSRepairJob *>(mock_thread_pool.jobQ.front().job)->node_id, 2);
    mock_thread_pool.thread_iteration();
    ASSERT_EQ(mock_thread_pool.jobQ.front().job->jobType, SVS_REPAIR_NODE_CONNECTIONS_JOB);
...
TYPED_TEST(SVSTieredIndexTestBasic, deleteVectorMultiFromFlatAdvanced) {
